...
realize (GtkGLArea *area)
{
  ...
  const char *fragment_shader_string = "#version 330 core\n"
    ...
    "uniform float uniChange;\n"
    "out vec4 FragColor;\n"
    "void main()\n"
    "{\n"
    "   FragColor = mix(texture(texture0, TexCoord), texture(texture1, TexCoord),uniChange);\n"
    "}\n\0";
   ...
   int uniChange = glGetUniformLocation(programID, "uniChange");
   change = 0.5f;
   glUniform1f(uniChange, change);
