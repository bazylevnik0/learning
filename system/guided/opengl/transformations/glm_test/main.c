...
GtkWidget *gl_area;
float uniShift, newShiftX, newShiftY, newShiftZ;
...
realize (GtkGLArea *area)
{
  ...
  const char *vertex_shader_string = "#version 330 core\n"
    ...
    "uniform vec3 uniShift;\n"
    "void main()\n"
    "{\n"
    "   gl_Position = vec4(aPos+uniShift, 1.0);\n"
    "}\0";
  const char *fragment_shader_string = "#version 330 core\n"
   ...
   newShiftX = newShiftY = newShiftZ = 0.0f;
...
render (GtkGLArea *area, GdkGLContext *context)
{
  ...
  uniShift = glGetUniformLocation(programID, "uniShift");
  glUseProgram(programID);
  glUniform3f(uniShift, newShiftX, newShiftY, newShiftZ);
...
button_press(GtkEventControllerKey *controller,
     ...
     gpointer                        data)
{
  transform_vec(&newShiftX, &newShiftY, &newShiftZ);
